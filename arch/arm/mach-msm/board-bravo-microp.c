...
static int gsensor_log_print(struct seq_file *sf, void *private)
{
	...
	for (n = gsensor_log_tail;
	     n != gsensor_log_head; n = (n + 1) & GSENSOR_LOG_MASK) {
...
int
bravo_microp_sdslot_status_register(void (*cb) (int card_present, void *dev_id),
				    void *dev_id)
{
	if (sdslot_status_cb)
		return -EBUSY;
					     ...
					     char *buf)
{
	...
	if (ldata->type == GREEN_LED) {
		ret = i2c_read_block(client,
				     MICROP_I2C_RCMD_GREEN_LED_REMAIN_TIME,
				     data, 2);
	} else if (ldata->type == AMBER_LED) {
		ret = i2c_read_block(client,
		...
		ret = i2c_read_block(client,
				     MICROP_I2C_RCMD_BLUE_LED_REMAIN_TIME, data,
				     2);
	} else {
...
int microp_spi_vote_enable(int spi_device, uint8_t enable)
{
	...
	if ((data[1] & 0x01) ==
		...
		printk(KERN_ERR
		       "%s: already in voted state, [spi_device %d,enable %d], [spi_status %d, spi_devices_vote %d]\n",
		       __func__, spi_device, enable, data[1] & 0x01,
		       cdata->spi_devices_vote);
		mutex_unlock(&cdata->microp_adc_mutex);
		return ret;
	}
...
static int gsensor_read_acceleration(short *buf)
{
	...
	ret = i2c_read_block(client, MICROP_I2C_RCMD_GSENSOR_DATA, tmp, 6);
	if (ret < 0) {
		dev_err(&client->dev, "%s: i2c_read_block fail\n", __func__);
		mutex_unlock(&gsensor_RW_mutex);
		return ret;
...
static int microp_i2c_suspend(struct i2c_client *client, pm_message_t mesg)
{
	return 0;
}
...
static struct {
	...
	[GREEN_LED] = {
		.name = "green",.led_set_work =
		    microp_led_brightness_set_work,.attrs =
		    green_amber_attrs,.attr_cnt = ARRAY_SIZE(green_amber_attrs)
	},[AMBER_LED] = {
		.name = "amber",.led_set_work =
		    microp_led_brightness_set_work,.attrs =
		    green_amber_attrs,.attr_cnt = ARRAY_SIZE(green_amber_attrs)
	},[BLUE_LED] = {
	.name = "blue",.led_set_work =
		    microp_led_brightness_gpo_set_work,.attrs =
		    NULL,.attr_cnt = 0},[BUTTONS_LED] = {
.name = "button-backlight",.led_set_work =
		    microp_led_buttons_brightness_set_work},};
			    ...
			    const struct i2c_device_id *id)
{
	...
	ret = request_irq(client->irq,
			  ...
			  IRQF_TRIGGER_LOW, "microp_interrupt", &client->dev);
	if (ret) {
